...
interface PendingInvoiceDao {
    ...
    suspend fun markSending(id: String, updatedAt: Long = System.currentTimeMillis())

    @Query("UPDATE pending_invoices SET status = 'SENT', remoteInvoiceId = :remoteInvoiceId, remoteInvoiceNumber = :remoteInvoiceNumber, lastError = NULL, updatedAt = :updatedAt WHERE id = :id")
    suspend fun markSent(id: String, remoteInvoiceId: String, remoteInvoiceNumber: String, updatedAt: Long = System.currentTimeMillis())

    @Query("UPDATE pending_invoices SET status = 'FAILED', retryCount = retryCount + 1, lastError = :message, updatedAt = :updatedAt WHERE id = :id")
    suspend fun markFailed(id: String, message: String, updatedAt: Long = System.currentTimeMillis())
}
