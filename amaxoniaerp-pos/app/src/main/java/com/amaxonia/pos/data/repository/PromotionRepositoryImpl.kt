...
class PromotionRepositoryImpl(
    ...
    override suspend fun getActivePromotionsForProduct(productId: String): Result<List<Promocion>> {
        return runCatching { promocionDao.getActiveByParentProduct(productId).map { it.toDomainPromotion() } }
    }

    override suspend fun getPromotionById(promotionId: String): Result<Promocion> {
        return runCatching { promocionDao.getById(promotionId)?.toDomainPromotion() ?: error("Promoción no encontrada") }
    }
