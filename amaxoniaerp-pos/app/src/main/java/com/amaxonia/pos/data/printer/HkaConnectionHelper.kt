 ...
 * - testConnection  â†’ TCPClientTest.testConnection() â€” plain socket connect, no encryption
 * - checkPrinterStatus â†’ MainController.checkStatus() â€” sends "05" encrypted, parses PrinterStatus
 */
class HkaConnectionHelper(
     ...
     * Matches SDK's TCPClientTest.testConnection() â€” simple socket connect + latency.
     */
                ...
                ConnectionTestResult(
                    ...
                    errorMessage =
                        when (e) {
                            ...
                            else -> e.message ?: "Error de conexiÃ³n desconocido"
                        },
                )
            }
        }
     ...
     *   2. setOutPutSteamByDevice(socket, "05") â€” encrypts the string
     *   3. getResponse() â€” reads response with byte-stripping
     *   4. PrinterStatus(bytes[0], bytes[1]) â€” parses status/error codes
     */
        ...
        withContext(ioDispatcher) {
            runCatching {
                Socket().use { socket ->
                    ...
                    // Encrypt and send "05" â€” matches SDK's checkStatus()
                    ...
                    val bytes =
                        encrypted.bytes
                            ?: error("Cifrado devolviÃ³ bytes nulos")
                ...
                PrinterStatusResult(
                    ...
                    errorMessage =
                        when (e) {
                            is java.net.SocketTimeoutException -> "Timeout: La impresora no respondiÃ³"
     ...
     *
     * SDK stripping logic:
     *   - If first byte of chunk is between 6 and 15 (inclusive) â†’ include all bytes (off=0)
     *   - Otherwise â†’ skip first byte (off=1)
     */
    private fun readResponseWithStripping(socket: Socket): ByteArray {
        ...
        try {
            ...
            // Expected â€” device stopped sending
        } finally {
