...
class CajaApiImpl(
    ...
    override suspend fun getCajas(
        authHeader: String,
        companyDb: String,
    ): Result<List<Caja>> =
        catchingResult {
            val response =
                apiClient.httpClient.get("api/cajas") {
    ...
    override suspend fun checkCajaStatus(
        cajaId: String,
        authHeader: String,
        companyDb: String,
        ...
        catchingResult {
            val response =
                apiClient.httpClient.get("api/cajas/$cajaId/status") {
                    header("Authorization", authHeader)
                    header("Company-DB", companyDb)
                }
    ...
    override suspend fun getCajaSecuencia(
        ...
        catchingResult {
            val response =
                apiClient.httpClient.get("api/cajas/secuencia") {
                    header("Authorization", authHeader)
                    header("Company-DB", companyDb)
            ...
            val parsed =
                runCatching {
                    ...
                    error("No se pudo obtener la secuencia de caja")
                }
    ...
    override suspend fun getNextSecuenciaCodigo(
        ...
        catchingResult {
            val response =
                apiClient.httpClient.get("api/cajas/secuencia/codigo") {
                    header("Authorization", authHeader)
                    header("Company-DB", companyDb)
                    parameter("id", idCaja)
                }
            ...
            val parsed =
                runCatching {
                    ...
                    error("No se pudo obtener el correlativo de secuencia")
                }
            Result.success(parsed)
        }

    override suspend fun openCaja(
        ...
        catchingResult {
            val response =
                apiClient.httpClient.post("api/cajas/open") {
                    header("Authorization", authHeader)
                    header("Company-DB", companyDb)
    ...
    override suspend fun closeCaja(
        ...
        catchingResult {
            val response =
                apiClient.httpClient.post("api/cajas/close") {
                    header("Authorization", authHeader)
                    header("Company-DB", companyDb)
                    contentType(ContentType.Application.Json)
                    setBody(request)
    ...
    private fun parseCajaStatusResponse(
        responseText: String,
