...
class ApiTransactionRepositoryTest {
    ...
    private val fakePendingDao =
        object : PendingInvoiceDao {
            ...
            override suspend fun getPendingForTenant(
                tenantId: String,
                limit: Int,
            ): List<PendingInvoiceEntity> = pendingEntities.filter { it.tenantId == tenantId }

            override suspend fun getCreatedBetween(
                fromMillis: Long,
                toMillis: Long,
            ): List<PendingInvoiceEntity> = emptyList()
    ...
    private val fakeSalesApi =
        object : SalesApi {
            override suspend fun processSale(
                authHeader: String,
                payload: ProcessSaleRequestDto,
            ): Result<ProcessSaleResponseDto> = error("Not used")
            ...
            override suspend fun getFacturaDetalle(
                authHeader: String,
                facturaId: String,
            ): Result<FacturaDetalleResponseDto> = Result.failure(IllegalStateException("Not found remotely"))

            override suspend fun findByCorrelationId(
                authHeader: String,
                clientCorrelationId: String,
            ): Result<ReconciledInvoice?> = error("Not used")
            ...
            override suspend fun getPrintPayload(
                authHeader: String,
                facturaId: String,
            ): Result<FacturaPrintPayloadDto> = error("Not used")

            override suspend fun sendReceiptEmail(
                authHeader: String,
                facturaId: String,
            ): Result<EnviarCorreoFacturaResponseDto> = error("Not used")
    ...
    @Test
    fun getTransactionsMergesPendingOfflineInvoicesWithRemoteInvoices() =
        runTest {
            val pendingSale =
    ...
    @Test
    fun getInvoiceDetailReturnsLocalItemDetailForPendingInvoice() =
        runTest {
            val pendingSale =
                ProcessSaleRequestDto(
                    idFactura = "OFF-200",
