...
class VenezuelaInvoiceStrategyTest {
    ...
    @Test
    fun `tipo_facturacion distinto de 5 ya NO retorna NotApplicable porque la seleccion HKA20 vive en el UseCase`() = runBlocking {
        ...
        val client = CountingClient(
            ...
            emision = VenezuelaHkaResponse(
                httpStatus = 200,
                rawBody = "{}",
                codigo = "200",
                mensaje = "OK",
                validaciones = emptyList(),
                resultado = VenezuelaHkaEmisionResponse(
                    codigo = "200",
                    mensaje = "El documento fue procesado con éxito.",
                    resultado = VenezuelaHkaEmisionResultado(
    ...
    @Test
    fun `factura ya emitida retorna AlreadyIssued sin llamar HKA`() = runBlocking {
        val repo = fakeRepo(
            tipoFacturacion = 5,
    ...
    @Test
    fun `idempotencia OR - solo numeroDocumentoFiscal presente retorna Failure PARTIAL y NO llama HKA`() = runBlocking {
        val repo = fakeRepo(
            tipoFacturacion = 5,
    ...
    @Test
    fun `idempotencia OR - solo numero_control_thka presente retorna Failure PARTIAL y NO llama HKA`() = runBlocking {
        val repo = fakeRepo(
            tipoFacturacion = 5,
    ...
    @Test
    fun `idempotencia OR - ambos ausentes continua el flujo de emision`() = runBlocking {
        val repo = fakeRepo(
            tipoFacturacion = 5,
    ...
    @Test
    fun `tipoDocumento distinto de 01 retorna UnsupportedDocumentType sin llamar HKA`() = runBlocking {
    ...
    @Test
    fun `emision exitosa retorna Success y persiste los tres campos fiscales`() = runBlocking {
    ...
    @Test
    fun `autenticacion rechazada 401 retorna Failure AUTH_REJECTED sin reservar correlativo`() = runBlocking {
        val repo = fakeRepo(tipoFacturacion = 5).apply { failReserveOnCall = true }
        val client = CountingClient(
            auth = VenezuelaHkaResponse(
                httpStatus = 401,
    ...
    @Test
    fun `codigo de negocio distinto de 200 retorna Failure y NO persiste`() = runBlocking {
        val repo = fakeRepo(tipoFacturacion = 5)
        val client = CountingClient(
            auth = okAuth(),
            ultimoDocumento = okUltimo("50"),
    ...
    @Test
    fun `timeout en emision retorna Uncertain y NO persiste ni reintenta`() = runBlocking {
        val repo = fakeRepo(tipoFacturacion = 5)
        val client = CountingClient(
            auth = okAuth(),
            ultimoDocumento = okUltimo("50"),
    ...
    @Test
    fun `timeout en autenticacion retorna Uncertain AUTH_NET_ERROR`() = runBlocking {
        val repo = fakeRepo(tipoFacturacion = 5)
        val client = CountingClient(
    ...
    @Test
    fun `correlativo local mayor que remoto usa el local`() = runBlocking {
    ...
    @Test
    fun `correlativo remoto mayor que local usa remoto_mas_uno`() = runBlocking {
    ...
    @Test
    fun `UltimoDocumento no concluyente cae al correlativo local sin fallar`() = runBlocking {
        val repo = fakeRepo(tipoFacturacion = 5, contadorInicial = 5, formato = 8)
        val client = CountingClient(
            auth = okAuth(),
            ultimoDocumento = VenezuelaHkaResponse(
                httpStatus = 404,
                rawBody = "",
                codigo = "HTTP_404",
    ...
    @Test
    fun `emision fallida con HTTP 400 no persista datos fiscales`() = runBlocking {
        val repo = fakeRepo(tipoFacturacion = 5)
        val client = CountingClient(
            auth = okAuth(),
            ultimoDocumento = okUltimo("50"),
    ...
    private fun strategy(repo: FakeRepo, client: VenezuelaHkaClient): VenezuelaInvoiceStrategy =
    ...
    private fun okAuth() = VenezuelaHkaResponse(
        httpStatus = 200,
        rawBody = """{"token":"jwt-1"}""",
        codigo = "200",
        mensaje = "OK",
    ...
    private fun okUltimo(num: String) = VenezuelaHkaResponse(
        httpStatus = 200,
        rawBody = """{"codigo":"200","resultado":{"ultimoNumero":"$num"}}""",
        codigo = "200",
        mensaje = "OK",
        validaciones = emptyList(),
        resultado = VenezuelaHkaUltimoDocumentoResponse(
            codigo = "200",
            mensaje = "OK",
    ...
    private fun okEmision(numDoc: String) = VenezuelaHkaResponse(
        httpStatus = 200,
    ...
    private class FakeRepo(
        ...
        override suspend fun loadInvoiceContext(database: Database, invoiceId: String): InvoiceVEContext = ctx

        override suspend fun loadAlreadyIssued(database: Database, invoiceId: String): AlreadyIssuedResult = alreadyIssued
         ...
         */
        override suspend fun reserveAtLeast(database: Database, minimumNextNumber: Int): VECorrelativoReservado {
    ...
    private class CountingClient(
        ...
        var authCalls = 0; private set
        var ultimoCalls = 0; private set
        var emissionCalls = 0; private set

        override suspend fun authenticate(credentials: PacCredentials) = applyThrow(authThrow) {
            authCalls += 1
        ...
        private inline fun <T> applyThrow(thrown: VenezuelaHkaClientException?, block: () -> T): T {
            if (thrown != null) throw thrown
            return block()
        }

        companion object {
            private fun defaultEmisionFail() = VenezuelaHkaResponse<VenezuelaHkaEmisionResponse>(
                httpStatus = 0,
                rawBody = "",
...
private fun sampleVEContext(tipoFacturacion: Int, tipoDocumento: String): InvoiceVEContext = InvoiceVEContext(
        config = VEConfigData(
            tipoFacturacion = tipoFacturacion,
            tipoEntornoVe = 0,
            tokenEmpresa = "U",
