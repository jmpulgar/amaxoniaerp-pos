...
class FacturasPrintPayloadDiscountTest {
    ...
    fun setUp() {
        database =
            Database.connect(
                url = "jdbc:h2:mem:facturas_print_${UUID.randomUUID().toString().replace("-", "")};MODE=MySQL;DB_CLOSE_DELAY=-1;DATABASE_TO_UPPER=false",
        ...
        transaction(database) {
            ...
            exec("CREATE TABLE IF NOT EXISTS sucursal (id INT, sucursal VARCHAR(100), descripcion VARCHAR(255), codigo_sucursal_emisor VARCHAR(40))")
            ...
            exec("CREATE TABLE IF NOT EXISTS caja_nueva_detalle (caja_detalle_id INT, caja_id VARCHAR(36), id_forma_pago INT, monto DECIMAL(20,2))")
