...
class ProcessSaleUseCaseSelectionTest {
    ...
    @Test
    fun `1 - VE con useHka20=true NO llama a la strategy digital y confirma la venta`() = runBlocking {
    ...
    @Test
    fun `2 - VE con useHka20=false SI llama a la strategy digital`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        // FASE 2 (Punto 1): Success VE usa campos propios (numeroDocumentoFiscal), nunca cufe.
        val digital = RecordingStrategy(
            "VE",
    ...
    @Test
    fun `3 - VE con useHka20=null (campo ausente) llama a la strategy digital - compatibilidad legacy`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val digital = RecordingStrategy(
            "VE",
    ...
    @Test
    fun `4 - PA con cualquier valor de useHka20 invoca a PA (campo ignorado)`() = runBlocking {
        val saleRepo = FakeSaleRepo()
    ...
    @Test
    fun `5 - VE con useHka20=true omite autenticacion, UltimoDocumento, emision y reserva digital`() = runBlocking {
    ...
    @Test
    fun `6 - VE con useHka20=true conserva success, codFactura y codEstatus en la respuesta`() = runBlocking {
    ...
    @Test
    fun `7 - VE con useHka20=true NO admite fallback al digital ante fallo posterior del HKA20`() = runBlocking {
    ...
    @Test
    fun `8 - VE con useHka20=false y digital fallido NO produce fallback al HKA20`() = runBlocking {
    ...
    @Test
    fun `9 - PA con tipo_facturacion bajo conserva su flujo - sin cambios por useHka20`() = runBlocking {
    ...
    @Test
    fun `10 - Idempotencia - el mismo request no puede ejecutar ambos mecanismos`() = runBlocking {
     ...
     */
    @Test
    fun `FASE 1_1 item 2 - reintento del mismo idFactura cambiando useHka20 a false no emite digitalmente`() = runBlocking {
        ...
        assertEquals(0, digital.processCalls,
     ...
     */
    @Test
    fun `FASE 1_1 item 5 - reintento del mismo idFactura cambiando useHka20 a true tampoco inicia HKA20`() = runBlocking {
        val saleRepo = DuplicateAwareSaleRepo()
        ...
        assertEquals(1, digital.processCalls,
    ...
    @Test
    fun `FASE 2 a - VE Success propaga numeroDocumentoFiscal desde el campo propio`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val digital = RecordingStrategy(
            "VE",
    ...
    @Test
    fun `FASE 2 b - VE Success propaga numeroControlThka desde el campo propio`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val digital = RecordingStrategy(
            "VE",
            result = ElectronicInvoiceResult.Success(numeroDocumentoFiscal = "00001234", numeroControlThka = "001-00001"),
    ...
    @Test
    fun `FASE 2 c - VE AlreadyIssued propaga los valores persistidos`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val digital = RecordingStrategy(
            "VE",
            result = ElectronicInvoiceResult.AlreadyIssued(country = "VE", numeroDocumentoFiscal = "00009988", numeroControl = "001-9988"),
        )
        val useCase = ProcessSaleUseCase(saleRepo, SingleStrategyFactory(digital))
    ...
    @Test
    fun `FASE 2 d - VE Success con numeroControlThka null deja numeroControlThka en null`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val digital = RecordingStrategy(
            "VE",
    ...
    @Test
    fun `FASE 2 e - VE HKA-20 deja los campos fiscales digitales en null`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val digital = RecordingStrategy("VE")
        val useCase = ProcessSaleUseCase(saleRepo, SingleStrategyFactory(digital))
    ...
    @Test
    fun `FASE 2 f - VE Failure NO inventa campos fiscales digitales`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val digital = RecordingStrategy(
            "VE",
    ...
    @Test
    fun `FASE 2 g - PA nunca popular numeroDocumentoFiscal ni numeroControlThka`() = runBlocking {
        val saleRepo = FakeSaleRepo()
    ...
    @Test
    fun `FASE 2 h - VE Uncertain no propaga campos fiscales - respuesta conservadora`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val digital = RecordingStrategy(
            "VE",
            result = ElectronicInvoiceResult.Uncertain(
                country = "VE",
    ...
    @Test
    fun `FASE 2 reg-PA-a - PA Success conserva cufe, qr, nroProtocoloAutorizacion y fechaRecepcionDGI`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val pa = RecordingStrategy(
            "PA",
            result = ElectronicInvoiceResult.Success(
                cufe = "CUFE-PA-1",
                qr = "QR-PA",
        ...
        assertEquals("PROTO-PA-1", result.nroProtocoloAutorizacion,
    ...
    @Test
    fun `FASE 2 reg-PA-b - VE Success jamas toca cufe ni nroProtocoloAutorizacion`() = runBlocking {
        val saleRepo = FakeSaleRepo()
        val digital = RecordingStrategy(
            "VE",
            result = ElectronicInvoiceResult.Success(
    ...
    private class RecordingStrategy(
        ...
        var processCalls = 0; private set
    ...
    private class ExplosiveStrategy(override val countryCode: String) : ElectronicInvoiceStrategy {
        override suspend fun processElectronicInvoice(database: Database, invoiceId: String): ElectronicInvoiceResult =
            error("El digital NO debió invocarse para este caso (useHka20=true o país no-VE)")
    }
     ...
     */
    private class SingleStrategyFactory(private val strategy: ElectronicInvoiceStrategy) :
        ProcessorFactory {
