...
class AuthRepository(private val database: Database) {
    suspend fun authenticate(username: String, passwordPlain: String): UserRecord? = dbQuery(database) {
    ...
    suspend fun loadUserById(userId: Int): UserRecord? = dbQuery(database) {
        UsersTable.selectAll()
