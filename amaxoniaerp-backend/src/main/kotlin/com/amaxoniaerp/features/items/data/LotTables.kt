...
object ConfiguracionLoteTable : Table("configuracion_lote") {
    ...
    val loteVence = varchar("lote_vence", S.VARCHAR_LENGTH_20).default("si")
...
object ItemLoteTable : Table("item_lote") {
    ...
    val idItem = integer("id_item")
    val codigoLoteItem = varchar("codigo_lote_item", S.VARCHAR_LENGTH_100)
    val vencimiento = date("vencimiento").nullable()
    val disponibilidad = decimal("disponibilidad", S.DECIMAL_PRECISION_10, 2).default(BigDecimal.ZERO)
    val procesamiento = decimal("procesamiento", S.DECIMAL_PRECISION_10, 2).default(BigDecimal.ZERO)
    val venta = decimal("venta", S.DECIMAL_PRECISION_10, 2).default(BigDecimal.ZERO)
