...
class CreditNoteService(
    ...
    suspend fun getInvoiceDetail(
        ...
        id: String,
        countryCode: String,
    ): CreditNoteSourceInvoiceDetailResponse? = dbQuery(database) {
    ...
    suspend fun create(
        ...
        return when (val result = processor.process(database, prepared, companyDb)) {
            ...
            is PanamaCreditNotePacResult.Rejected -> dbQuery(database) {
                repository.markPanamaFiscalStatus(
                    id = prepared.id,
            ...
            is PanamaCreditNotePacResult.Uncertain -> dbQuery(database) {
                repository.markPanamaFiscalStatus(
                    id = prepared.id,
                    status = CreditNoteFiscalStatus.INCIERTA,
                    message = "${result.codigo}: ${result.mensaje}",
