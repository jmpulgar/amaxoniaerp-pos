...
suspend fun getBestSellerItemQuantities(
    ...
    dbQuery(database) {
        ...
        TransactionManager.current().exec(
            "SELECT id_item, SUM(_item_cantidad) AS total FROM factura_detalle " +
                "WHERE (anulado = 0 OR anulado IS NULL) AND id_item IS NOT NULL " +
                "GROUP BY id_item ORDER BY total DESC LIMIT $safeLimit",
        ) { result ->
