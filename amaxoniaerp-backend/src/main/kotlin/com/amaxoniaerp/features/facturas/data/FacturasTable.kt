...
abstract class BaseFacturasTable(
    ...
    val totalTotalFactura = decimal("TotalTotalFactura", SchemaDimensions.DECIMAL_PRECISION_10, 2).default(0.0.toBigDecimal())
    val totalizarTotalGeneral = decimal("totalizar_total_general", SchemaDimensions.DECIMAL_PRECISION_10, 2).default(0.0.toBigDecimal())
...
object FacturasTableVE : BaseFacturasTable() {
    ...
    val abrMonedaSecundaria = varchar("abr_moneda_secundaria", SchemaDimensions.VARCHAR_LENGTH_10).nullable()
...
object FacturasTablePA : BaseFacturasTable() {
    ...
    val fechaRecepcionDGI = varchar("fechaRecepcionDGI", SchemaDimensions.VARCHAR_LENGTH_25).nullable()
    val cufe = text("cufe").nullable()
    val qr = text("qr").nullable()
    val nroProtocoloAutorizacion = varchar("nroProtocoloAutorizacion", SchemaDimensions.VARCHAR_LENGTH_100).nullable()
    val fechaLimite = varchar("fechaLimite", SchemaDimensions.VARCHAR_LENGTH_25).nullable()
}
...
object FacturasClientesTable : Table("clientes") {
    ...
    val apellido = varchar("apellido", SchemaDimensions.VARCHAR_LENGTH_100).nullable()
    val rif = varchar("rif", SchemaDimensions.VARCHAR_LENGTH_20)
...
object EstatusTable : Table("estatus") {
    ...
    val descripcion = varchar("descripcion", SchemaDimensions.VARCHAR_LENGTH_50)
