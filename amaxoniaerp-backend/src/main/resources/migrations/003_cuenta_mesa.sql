...
-- -----------------------------------------------------------------------------
-- (1) Cabecera de cuenta: una sesión puede tener N cuentas consecutivas
...
-- -----------------------------------------------------------------------------
-- (2) Detalle de cuenta: cada fila es una porción facturable de un pedido.
--     Permite división por producto (cantidad = pedido.item_cantidad) o división
--     parcial por cantidad (cantidad < pedido.item_cantidad).
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `cuenta_mesa_detalle` (
    ...
    -- Snapshots: precio y porcentaje de IVA del pedido original.
    ...
    CONSTRAINT `fk_cuenta_detalle_cuenta`   FOREIGN KEY (`cuenta_mesa_id`) REFERENCES `cuenta_mesa` (`id`),
    CONSTRAINT `fk_cuenta_detalle_pedido`   FOREIGN KEY (`pedido_mesa_id`) REFERENCES `pedido_mesa` (`id`),
    CONSTRAINT `fk_cuenta_detalle_producto` FOREIGN KEY (`producto_id`)   REFERENCES `item` (`id_item`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- -----------------------------------------------------------------------------
-- (3) Idempotencia: clave única por intento de cobro. Previene dobles toques,
--     reintentos, timeouts y reconexiones.
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `cuenta_mesa_idempotencia` (
    ...
    `id_factura_resultado` VARCHAR(36)    NULL,
    `cod_factura_resultado` VARCHAR(32)   NULL,
    ...
    CONSTRAINT `fk_cuenta_idem_cuenta` FOREIGN KEY (`cuenta_mesa_id`) REFERENCES `cuenta_mesa` (`id`),
    CONSTRAINT `fk_cuenta_idem_sesion` FOREIGN KEY (`sesion_mesa_id`) REFERENCES `sesion_mesa` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
